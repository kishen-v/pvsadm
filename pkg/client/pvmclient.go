...
func NewPVMClient(c *Client, instanceID, instanceName string, ep map[string]string) (*PVMClient, error) {
	...
	if len(workspaces.Resources) == 0 {
		return nil, fmt.Errorf("no resource instances are availble to be listed for ID: %s", instanceID)
	}
	pvmclient := &PVMClient{
...
func NewGenericPVMClient(c *Client, instanceID string, ep map[string]string, session *ibmpisession.IBMPISession) (*PVMClient, error) {
	...
	workspace, _, err := c.ResouceControllerClient.GetResourceInstance(getServiceInstanceOptions)
	if err != nil {
		return nil, fmt.Errorf("failed to list the resource instances: %v", err)
	}
